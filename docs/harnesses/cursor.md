...
| `skill_activated`, `hook_execution_complete`, `cloud_agent_*`, `plugin_*` | root span events (name only) |
...
even though its detail attributes are gone.
...
| `cursor.model.name` | `gen_ai.request.model` | mapped (also names the span) |
...
| any duration/latency field | — | not provided (the wire carries no durations ⇒ no TTFT source) |

### invoke_agent cursor root

| Raw key | Canonical key | Status |
|---|---|---|
| `service.version` (resource) | `coding_agent.client.version` | mapped |
...
| aggregate usage rollup (turn-total token counts on the root) | — | dropped (usage lives on chat spans only; sum them for turn totals) |
...
All of these used to carry `coding_agent.cursor.*` copies of their wire
attributes; every one is now dropped and the events keep names and timestamps
only:
...
| `coding_agent.cursor.plugin.name` | `cursor.plugin.name` | dropped (event name survives) |
...
- `gen_ai.agent.name` = `cursor` (invoke_agent root)
