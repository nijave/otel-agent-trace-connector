...
- **Usage capture:** keep xeonvs's `6085090` from the merge; diff it against
  our `fix/usage-capture-on-content-chunk` and cherry-pick ours on top only
  if it covers chunk shapes theirs misses. Upstream PR CallOrRet#4 stays
  open regardless — that is upstream's queue, not ours.
